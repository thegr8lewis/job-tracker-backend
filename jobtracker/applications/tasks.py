...
def fetch_jobs_periodic():
    ...
    for f in filters:
        providers = f.providers or ["jooble", "adzuna"]
        logger.info(
            f"Processing filter for user {f.user_uid}: "
            f"keywords={f.keywords}, locations={f.locations}, providers={providers}"
        )
                ...
                for j in jobs:
                    ...
                    with transaction.atomic():
                        ...
                        if was_created:
                            total_new_apps += 1
                            logger.info(
                                f"Created new application: {title} at {company} "
                                f"for user {f.user_uid}"
                            )

    logger.info(
        f"Periodic job fetch completed. "
        f"New Jobs: {total_new_jobs} | New Saved Applications: {total_new_apps}"
    )
    return {"jobs": total_new_jobs, "applications": total_new_apps}
...
def process_user_emails(user_uid: str):
    """
    Process Gmail job-related emails for a user.
    Creates/updates EmailLog, TimelineEvent, and Application status.
    """
    try:
        try:
            email_settings = UserEmailSettings.objects.get(
                user_uid=user_uid, gmail_connected=True
            )
        except UserEmailSettings.DoesNotExist:
            logger.info(f"Email tracking not enabled for user {user_uid}")
            return {"fetched": 0, "processed": 0, "updated": 0}
        ...
        logger.info(f"Found {fetched_count} potential job-related emails for {user_uid}")
        ...
        for email_data in emails:
            try:
                ...
                application = gmail_service.match_email_to_application(user_uid, email_data)
                if not application:
                    logger.debug(f"No application match for email {email_data['email_id']}")
                    continue
                ...
                analysis = ai_service.analyze_job_email(email_data, app_data)
                if not analysis:
                    logger.warning(f"AI analysis failed for {email_data['email_id']}")
                    continue

                # Create or update EmailLog safely
                email_log, created = EmailLog.objects.update_or_create(
                    email_id=email_data["email_id"],
                    defaults={
                        "application": application,
                ...
                # Create timeline event for email if not exists
                if not TimelineEvent.objects.filter(
                    application=application, email_log=email_log, event_type="email_received"
                ).exists():
                    TimelineEvent.objects.create(
                        application=application,
                        event_type="email_received",
                        title="Email Received",
                        description=(
                            f"From: {email_data['sender_email']}\n"
                            f"Subject: {email_data['subject']}\n\n"
                            f"AI Analysis: {analysis.get('timeline_note', '')}"
                        ),
                        date=timezone.localdate(),
                        completed=True,
                        is_auto_generated=True,
                ...
                # Update application status if AI suggests
                new_status = analysis.get("suggested_status")
                if new_status and new_status != application.status:
                    application.status = new_status
                    application.save()
                    ...
                    TimelineEvent.objects.create(
                        ...
                        title=f"Status updated to {new_status}",
                        description=(
                            f"Automatically updated based on email from "
                    ...
                    logger.info(
                        f"Updated application {application.id} for user {user_uid} "
                        f"status → {new_status}"
                    )

            except Exception as e:
                logger.error(
                    f"Error processing email {email_data.get('email_id', 'unknown')}: {str(e)}"
                )
                continue

        logger.info(
            f"Completed email processing for {user_uid}: "
            f"{fetched_count} fetched, {processed_count} new, {updated_count} updated"
        )
        return {
        ...
        logger.error(f"Fatal error processing emails for {user_uid}: {str(e)}")
        return {"fetched": 0, "processed": 0, "updated": 0}
...
def process_all_users_emails():
    """
    Run email processing for all Gmail-connected users.
    """
    users_with_email = UserEmailSettings.objects.filter(
        gmail_connected=True, email_tracking_enabled=True
    ).values_list("user_uid", flat=True)
    ...
    logger.info(
        f"Completed email processing for all users: "
        f"{total_fetched} fetched, {total_processed} new, {total_updated} updated"
    )
    return {
